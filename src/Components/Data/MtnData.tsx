...
export const MtnData = () => {
    ...
    const [data,setData] = useState<{ varCode: string, biller: string }>({ varCode: "mtn-10mb-100", biller: "" })
    ...
    const handleVars = async () => {
        try {
            const data = await getDataVars("mtn-data")
            if (data) {
    ...
    return (
        <Box mt={"1rem"}>
            <Heading textAlign={"center"} fontSize={"1.1rem"} mb={"2rem"}>mtn data</Heading>
