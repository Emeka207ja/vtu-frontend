...
export const AirtelData = () => {
    ...
    const [data,setData] = useState<{ varCode: string, biller: string }>({ varCode: "Airtel Data Bundle - 50 Naira - 25MB  - 1Day", biller: "" })
    const [amt, setAmt] = useState<string>("49.99")
    ...
    const handleVars = async () => {
        try {
            const data = await getDataVars("airtel-data")
            if (data) {
    ...
    return (
        <Box mt={"1rem"}>
            <Heading textAlign={"center"} fontSize={"1.1rem"} mb={"2rem"}>Airtel data</Heading>
